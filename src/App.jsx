...
const faqs = [
  {
    q: "Are these sessions beginner friendly?",
    a: "Yes. We start from zero, so no prior experience is required.",
  },
  {
    q: "What if we miss a class?",
    a: "All classes are recorded and make-up sessions are available.",
  },
  {
    q: "Will this help with school exams?",
    a: "Yes. Students learn concepts practically and create smarter study notes.",
  },
  {
    q: "What is the batch size?",
    a: "Each batch has a maximum of 15 students.",
  },
];
...
export default function App() {
  ...
  return (
    <div id="top" className="min-h-screen bg-brand-50 text-slate-900">
      <Header />
      <main>
        <Hero />
            ...
            <div className="mt-5 space-y-4">
              {faqs.map((faq) => (
                <details key={faq.q} className="overflow-hidden rounded-2xl border border-slate-200 bg-[#F3F4F6]">
                  <summary className="cursor-pointer list-none border-b border-slate-200 px-6 py-5 text-2xl font-semibold text-slate-900/95 md:text-[2rem]">
                    {faq.q}
                  </summary>
                  <p className="px-6 py-6 text-xl leading-relaxed text-slate-800 md:text-[2rem]">{faq.a}</p>
                </details>
              ))}
            </div>
          </div>
      ...
      <div className="fixed bottom-5 right-5 z-[60] flex flex-col items-center gap-2">
